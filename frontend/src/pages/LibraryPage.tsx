...
function BookCard({
  ...
  const cardClass = "block bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow";

  return (
    <div className="relative group">
      {book.is_saved ? (
        <Link to={`/books/${book.id}`} className={cardClass}>{cardContent}</Link>
      ) : (
