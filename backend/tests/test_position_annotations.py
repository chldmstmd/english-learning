...
from app.models.paragraph import ArticleParagraph, ParagraphAnnotation
    ...
    async def scenario(session_factory):
        async with session_factory() as db:
            cols = await db.execute(text(
                ...
                "WHERE table_name = 'paragraph_annotations'"
            ))
            names = {r[0] for r in cols}
            assert {"article_paragraph_id", "sentence_index", "word_index", "is_stale"} <= names

            uniq = await db.execute(text(
                "SELECT constraint_name FROM information_schema.table_constraints "
                "WHERE table_name = 'paragraph_annotations' AND constraint_type = 'UNIQUE'"
            ))
            assert "uq_paragraph_annotation_position" in {r[0] for r in uniq}
...
async def _seed_article(session_factory, article_id, user_id):
    async with session_factory() as db:
        ...
        db.add(article)
        await db.flush()
        await paragraph_service.replace_article_paragraphs(db, article, "by the bank of the river")
        link = await db.scalar(
            select(ArticleParagraph).where(ArticleParagraph.article_id == article_id)
        )
        await db.commit()
...
async def _cleanup_anns(session_factory, user_id):
    async with session_factory() as db:
        await db.execute(delete(ParagraphAnnotation).where(ParagraphAnnotation.user_id == user_id))
...
def test_same_lemma_different_positions_are_independent():
    ...
    async def scenario(session_factory):
        try:
            link_id = await _seed_article(session_factory, aid, uid)
            async with session_factory() as db:
                await annotation_service.upsert_annotation(
                    db, aid, uid, link_id, 'bank', sentence_index=0, word_index=3,
                    translation='河岸', source_sentence='by the bank of the river',
                )
                await annotation_service.upsert_annotation(
                    db, aid, uid, link_id, 'bank', sentence_index=1, word_index=9,
            ...
            assert anns[f'{link_id}-0-3']['translation'] == '河岸'
            assert anns[f'{link_id}-1-9']['translation'] == '银行'
        finally:
        ...
        try:
            ...
            async with session_factory() as db:
                ann = await annotation_service.upsert_annotation(
                    db, aid, uid, link_id, 'bank', sentence_index=0, word_index=3, translation='河岸',
                )
                ann.is_stale = True
                await db.commit()
            ...
            async with session_factory() as db:
                anns = await annotation_service.get_article_annotations(db, aid, uid)
            assert f'{link_id}-0-3' not in anns
        finally:
            await _cleanup_anns(session_factory, uid)
...
def test_translate_endpoint_writes_position_annotation():
    ...
    async def scenario(session_factory):
        try:
            ...
            async with session_factory() as db:
                await annotation_service.upsert_annotation(
                    db, aid, uid, link_id, "bank", sentence_index=0, word_index=1,
                    translation="银行", source_sentence="the bank",
                )
                await db.commit()
            async with session_factory() as db:
                anns = await annotation_service.get_article_annotations(db, aid, uid)
            assert anns[f"{link_id}-0-1"]["translation"] == "银行"
        finally:
            await _cleanup_anns(session_factory, uid)
