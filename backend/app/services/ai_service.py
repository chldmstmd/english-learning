...
def _build_sentence_blocks(
    ...
    Each word is advertised as `"{si}_{wi}: word`, i.e. the exact "si_wi" key
    the caller reads results back by, paired with the word so the model knows
    what to translate. Keeping the advertised identifier identical to the
    lookup key is the whole point — a mismatch silently drops every result.
    """
    ...
    for si in sorted(by_sentence):
        ...
        words_repr = ", ".join(f'"{si}_{wi}": {text}' for wi, text in sorted(by_sentence[si]))
...
async def batch_translate_article(
    ...
    prompt = _BATCH_TRANSLATION_PROMPT.format(
        ...
        sentence_blocks=_build_sentence_blocks(word_entries, sentences),
    )
    provider = _get_provider()
